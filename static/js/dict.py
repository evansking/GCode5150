...
dictionary = {}
for cmd, desc in common_comm.items():
    dictionary[cmd] = {
        'description': {'normal': desc},
        'parameters': {}
    }
    if cmd in simp_comm.items():
        dictionary[cmd]['description']['simple'] = simp_comm[cmd]
...
for param in params.keys():
    dct = params[param]
    for cmd, desc in dct[0].items():
        dictionary[cmd]['parameters'][param] = {'normal': desc}
    for cmd, desc in dct[1].items():
        dictionary[cmd]['parameters'][param]['simple'] = desc

g = []
m = []

for cmd in dictionary.keys():
    num = 0
    try:
        num = int(cmd[1:])
    except:
